...
<xsl:stylesheet version="3.0"
    ...
    xmlns:cf="http://idActs/AOGROOKS.xsd">
    <xsl:output method="html" omit-xml-declaration="yes" />

    <xsl:template match="/">

        <xsl:apply-templates select="/cf:aogrooks" />

    </xsl:template>

    <xsl:template match="/cf:aogrooks">
        ...
        <html>
            ...
            <body>
                <xsl:for-each select="cf:actInfo">
                    <h1>АКТ <br>освидетельствования геодезической разбивочной основы объекта капитального строительства</br>
                    </h1>
                    ...
                    <xsl:if test="ct:workExecutorRepresentative">
                        <p class="list">Представитель лица, выполнившего работы по созданию геодезической разбивочной основы</p>
                    ...
                    <p class="list">рассмотрели представленную документацию на геодезическую разбивочную основу для строительства:</p>
                    <p class="data">
                        <xsl:value-of select="ct:permanentObjectInfo/ct:permanentObjectName"/>
                    </p>
                    <p class="under">наименование объекта капитального строительства</p>
                    <p class="list">Предъявленные к освидетельствованию знаки геодезической разбивочной основы для строительства, их координаты, отметки, места установки и способы закрепления соответствуют требованиям проектной документации, а также техническим регламентам, иным нормативным правовым актам</p>
                    <xsl:for-each select="cf:geodeticSurveyMarkersInspection">
                        ...
                        <xsl:for-each select="cf:ntd/ct:technicalRegulationDetailsList/ct:technicalRegulationDetailsListItem">
                            <p class="data">
                                <xsl:value-of select="ct:name"/>, 
                                <xsl:value-of select="ct:number"/>
                                <xsl:choose>
                                    <xsl:when test="ct:technicalRegulationUnitsList">
                                        (
                    ...
                    <p class="list">и выполнены с соблюдением заданной точности построений и измерений.</p>
                    ...
                    <xsl:if test="cf:attachmentsList">
                        ...
                        <xsl:for-each select="cf:attachmentsList/cf:attachmentsListItem">
                            <p class="data">
                                <xsl:choose>
                                    <xsl:when test="cf:asBuiltSchema">
                                        <xsl:for-each select="cf:asBuiltSchema">
                                            <xsl:value-of select="ct:name"/> №
                                            <xsl:value-of select="ct:number"/> 
                                    ...
                                    <xsl:when test="cf:coordinatesRecords">
                                        <xsl:for-each select="cf:coordinatesRecords">
                                            <xsl:value-of select="ct:name"/> №
                                            <xsl:value-of select="ct:number"/> 
                                            <xsl:if test="ct:date">
                                                от 
