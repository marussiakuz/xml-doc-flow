...
<xsl:stylesheet version="3.0"
    ...
    xmlns:cf="http://v_17_0/idJournals/idSpJournalConcreteWorks.xsd">
    <xsl:output method="html" omit-xml-declaration="yes" />
    <xsl:param name="version" select="1.0"/>
    
    <xsl:template match="/">
        <xsl:apply-templates select="/cf:idSpJournalConcreteWorks" />
    </xsl:template>
    
    <xsl:template match="/cf:idSpJournalConcreteWorks">      
        ...
        <html>
            ...
            <body>
                <xsl:for-each select="cf:buildingContractorFinishSignedPart">
                    <xsl:for-each select="cf:buildingContractorSignedPart">
                       ...
                       <xsl:if test="../cf:journalLogEndDate">
                            <div class="str">
                                <p class="list">Ведение журнала: окончание </p>
                                <p class="data">
                                    <xsl:call-template name="formatdate">
                                        <xsl:with-param name="DateStr" select="../cf:journalLogEndDate" />
                                    </xsl:call-template>
                                </p>
                            </div>
                        </xsl:if>
                        <div class="str">
                            <p class="list">Количество записей в журнале бетонных работ </p>
                            <p class="data">
                                <xsl:value-of select="cf:journalVolume"/>
                            </p>
                        </div>
                    ...
                    <xsl:for-each select="cf:journalEndMark">
                        <div style="border: 1px solid; padding: 20px; margin: 20 0; float: right">Работы завершены. Журнал закрыт.</div>
                    </xsl:for-each>
                </xsl:for-each><br/>
            </body>
        </html>
