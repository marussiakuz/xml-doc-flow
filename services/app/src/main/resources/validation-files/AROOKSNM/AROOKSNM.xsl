...
<xsl:stylesheet version="3.0"
    ...
    xmlns:cf="http://idActs/AROOKSNM.xsd">
    <xsl:output method="html" omit-xml-declaration="yes" />

    <xsl:template match="/">

        <xsl:apply-templates select="/cf:arooksnm" />

    </xsl:template>

    <xsl:template match="/cf:arooksnm">
        ...
        <html>
            ...
            <body>
                <xsl:for-each select="cf:actInfo">
                    <h1>
                        АКТ 
                        <br>разбивки осей объекта капитального строительства на местности</br>
                    </h1>
                    ...
                    <xsl:if test="ct:workExecutorRepresentative">
                        <p class="list">Представитель лица, выполнившего работы по разбивке осей объекта капитального строительства на местности</p>
                    ...
                    <p class="list">составили настоящий акт о том, что произведена в натуре разбивка осей</p>
                    <xsl:for-each select="cf:inspectionObject/cf:axes">
                        <xsl:choose>
                            <xsl:when test="cf:obligatoryLiteralAxesList">
                                <xsl:for-each select="cf:obligatoryLiteralAxesList">
                                    <xsl:for-each select="cf:literalAxesList">
                                        <p class="data">
                                            <xsl:value-of select="cf:literalAxesListItem"/>
                                        </p>
                                    </xsl:for-each>
                                    <xsl:if test="cf:numericAxesList">
                                        <xsl:for-each select="cf:numericAxesList">
                                            <p class="data">
                                                <xsl:value-of select="cf:numericAxesListItem"/>
                                            </p>
                                        </xsl:for-each>
                                    </xsl:if>
                                </xsl:for-each>
                            </xsl:when>
                            <xsl:when test="cf:obligatoryNumericAxesList">
                                <xsl:for-each select="cf:obligatoryNumericAxesList">
                                    <xsl:if test="cf:literalAxesList">
                                        <xsl:for-each select="cf:literalAxesList">
                                            <p class="data">
                                                <xsl:value-of select="cf:literalAxesListItem"/>
                                            </p>
                    ...
                    <p class="list">объекта капитального строительства</p>
                    <p class="data">
                        <xsl:value-of select="ct:permanentObjectInfo/ct:permanentObjectName"/>
                    </p>
                    <p class="under">наименование объекта капитального строительства</p>
                    <p class="list">При этом установлено:</p>
                    <p class="list">1. Разбивка произведена по данным</p>
                    ...
                    <p class="under">номер, другие реквизиты чертежа, наименование проектной документации</p>
                    <p class="list">2. Закрепление осей произведено</p>
                    <xsl:for-each select="cf:workAndProjectDocumentationsList/cf:workAndProjectDocumentationsListItem">
                    ...
                    <p class="list">3. Обозначение осей, нумерация и расположение точек соответствуют проектной документации.</p>
                    <p class="list">Разбивка осей объекта капитального строительства на местности соответствует требованиям проектной документации, а также техническим регламентам, иным нормативным правовым актам</p>
                    <xsl:for-each select="cf:axesLayoutRequirementsDocs/cf:ntd/ct:technicalRegulationDetailsList/ct:technicalRegulationDetailsListItem">
                        <p class="data">
                            <xsl:value-of select="ct:name"/><xsl:text> </xsl:text>
                            <xsl:value-of select="ct:number"/>
                            <xsl:if test="ct:technicalRegulationUnitsList">
                                (
                                <xsl:for-each select="ct:technicalRegulationUnitsList/ct:technicalRegulationUnitsListItem">
                                    <xsl:value-of select="ct:name"/><xsl:text> </xsl:text>
                                    <xsl:value-of select="ct:number"/>
                                </xsl:for-each>)
                            </xsl:if>
                    ...
                    <p class="under">наименования и структурные единицы технических регламентов, иных нормативных правовых актов</p>
                    <p class="list">и выполнена с соблюдением заданной точности построений и измерений.</p>
                    <br/>
                    ...
                    <xsl:if test="cf:asBuiltSchemasList">
                        <p class="list">Приложения:</p>
                        <xsl:for-each select="cf:asBuiltSchemasList/cf:asBuiltSchemasListItem">
                            <p class="data">
