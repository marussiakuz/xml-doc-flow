...
<xsl:stylesheet version="3.0"
    ...
    xmlns:cf="http://idActs/AOUSITO.xsd">
    <xsl:output method="html" omit-xml-declaration="yes" />

    <xsl:template match="/">

        <xsl:apply-templates select="/cf:aousito" />

    </xsl:template>

    <xsl:template match="/cf:aousito">
        ...
        <html>
            ...
            <body>
                <xsl:for-each select="cf:actInfo">
                    ...
                    <h2>Организация, осуществляющая эксплуатацию сетей инженерно-технического обеспечения</h2>
                    <xsl:choose>
                        <xsl:when test="cf:utilitySystemsOperator">
            ...
            <body>
                <xsl:for-each select="cf:actInfo">
                    <h1>АКТ <br>освидетельствования участков сетей инженерно-технического обеспечения</br></h1>
                    ...
                    <xsl:if test="ct:workExecutorRepresentative">
                        <p class="list">Представители лица, выполнившего участки сетей инженерно-технического обеспечения</p>
                    ...
                    <if select="cf:utilitySystemsOperatorRepresentative">
                        <p class="list">Представитель организации, осуществляющей эксплуатацию сетей инженерно-технического обеспечения</p>
                    ...
                    <p class="list">составили настоящий акт о нижеследующем:</p>
                    <!--1. К освидетельствованию предъявлены следующие ответственные конструкции:-->
                    <p class="list">1. К освидетельствованию предъявлены следующие участки сети инженерно-технического обеспечения:</p>
                    <xsl:for-each select="cf:inspectedUtilitySystemsSectionsList/cf:inspectedUtilitySystemsSectionsListItem">
                        <p class="data">
                            <xsl:value-of select="cf:utilitySystemsSectionName"/> <xsl:text> -</xsl:text>
                            <xsl:value-of select="cf:location"/>
                        </p>
                    </xsl:for-each>
                    <p class="under">перечень и краткая характеристика участков сетей инженерно-технического обеспечения</p>
                    <p class="list">2. Участки сетей инженерно-технического обеспечения выполнены по проектной документации</p>
                    ...
                    <p class="list">3. Технические условия подключения объекта капитального строительства к сетям инженерно-технического обеспечения предоставлены</p>
                    <xsl:choose>
                        <xsl:when test="cf:technicalConditions">
                            <p class="data">
                                <xsl:for-each select="cf:technicalConditions">
                                    <xsl:value-of select="cf:technicalConditionsDoc/ct:number"/> от 
                                    <xsl:call-template name="formatdate">
                                        <xsl:with-param name="DateStr" select="cf:technicalConditionsDoc/ct:date" />
                                    </xsl:call-template>, 
                                    <xsl:choose>
                                        <xsl:when test="cf:technicalConditionsOrganization/ct:individualEntrepreneur">
                                            выдан 
                                            <xsl:value-of select="cf:technicalConditionsOrganization/ct:individualEntrepreneur"/>
                                        </xsl:when>
                                        <xsl:when test="cf:technicalConditionsOrganization/ct:legalEntity">
                                            выдан 
                                            <xsl:value-of select="cf:technicalConditionsOrganization/ct:legalEntity"/>
                                        </xsl:when>
                                    </xsl:choose>
                                    <xsl:choose>
                                        <xsl:when test="cf:technicalConditionsDoc/ct:expirationDate">
                                            до 
                                            <xsl:call-template name="formatdate">
                                                <xsl:with-param name="DateStr" select="cf:technicalConditionsDoc/ct:expirationDate" />
                                            </xsl:call-template>
                                        </xsl:when>
                                    </xsl:choose>
                                    <xsl:choose>
                                        <xsl:when test="cf:additionalInfo">
                                            Дополнительная информация: 
                                            <xsl:value-of select="cf:additionalInfo"/>
                                        </xsl:when>
                                    </xsl:choose>
                                </xsl:for-each>
                            </p>
                    ...
                    <p class="under">номер и дата технических условий, кем выданы, срок действия технических условий, иные сведения</p>
                    <p class="list">4. Освидетельствованы скрытые работы, оказывающие влияние на безопасность участков сетей инженерно-технического обеспечения:</p>
                    <xsl:choose>
                        <xsl:when test="cf:hiddenWorksList">
                            <xsl:for-each select="cf:hiddenWorksList/cf:hiddenWorksListItem">
                                <p class="data">
                    ...
                    <p class="list">5. При выполнении участков сетей инженерно-технического обеспечения применены:</p>
                    ...
                    <p class="list">6. Предъявлены документы, подтверждающие соответствие участков сетей инженерно-технического обеспечения предъявляемым к ним требованиям, в том числе:</p>
                    <xsl:for-each select="cf:requirementsComplianceConfirmationInfo">
                        <xsl:for-each select="cf:requirementsComplianceConfirmationDocuments">
                            <xsl:choose>
                        ...
                        <p class="list">технические условия</p>
                        <xsl:for-each select="cf:technicalConditionsDoc">
                            <p class="data">
                                <xsl:value-of select="ct:name"/><xsl:text> </xsl:text>
                                <xsl:value-of select="ct:number"/><xsl:text> от </xsl:text>
                                <xsl:call-template name="formatdate">
                    ...
                    <p class="list">7. Проведены необходимые испытания и опробования</p>
                    <xsl:choose>
                    ...
                    <p class="list">9. Предъявленные участки сетей инженерно-технического обеспечения выполнены в соответствии с техническими условиями подключения, техническими регламентами, иными нормативными правовыми актами и проектной документацией</p>
