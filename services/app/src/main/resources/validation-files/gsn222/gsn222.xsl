...
<xsl:stylesheet version="3.0"
    ...
    xmlns:cf="http://gsn/gsn222.xsd">
    <xsl:output method="html" omit-xml-declaration="yes" />

    <xsl:template match="/">

        <xsl:apply-templates select="/cf:gsn222" />

    </xsl:template>

    <xsl:template match="/cf:gsn222">
                ...
                <xsl:for-each select="cf:controlledPersonSignedPart">
                    <xsl:for-each select="cf:gsnInfo">
                        ...
                        <xsl:for-each select="cf:actInfo">
                            ...
                            <xsl:for-each select="cf:decisionDocument">
                                <p class="list">
                                ...
                                <p class="under">
                                    ...
                                    номер документарной проверки в едином реестре контрольных 
                                    (надзорных) мероприятий)
                                </p>
                            </xsl:for-each>
                            <p class="list">
                                2. Документарная проверка проведена в рамках
                            </p>
                            <p class="data">
                            ...
                            <p class="under">
                                ...
                                вида муниципального контроля в соответствии с единым реестром видов 
                            ...
                            <xsl:for-each select="cf:authorizedPersonsList">
                                ...
                                <p class="under">
                                    ...
                                    проведение документарной проверки. При замене инспектора 
                                    (инспекторов) после принятия решения о проведении документарной 
                                    проверки такой инспектор (инспекторы) указывается (указываются), 
                                    если его (их) замена была проведена после начала документарной проверки)
                                </p>
                            </xsl:for-each>
                            <p class="list">
                                ...
                                <xsl:choose>
                                    ...
                                    <xsl:when test="cf:involvedExpertsList">
                                        ...
                                        <p class="under">
                                            (указываются фамилии, имена, отчества (при наличии), 
                                            должности экспертов, с указанием сведений об аттестации 
                                            эксперта в реестре экспертов контрольного 
                                            (надзорного) органа)
                                        </p>
                                    </xsl:when>
                                    <xsl:otherwise>
                            ...
                            <xsl:for-each select="cf:permanentObjectInfo">
                                ...
                                <p class="under">
                                    ...
                                    которого проведена документарная проверка)
                                </p>
                                <p class="list">
                                    6. Документарная проверка была проведена 
                                ...
                                <p class="under">
                                    ...
                                    контролируемым лицом деятельности или места нахождения иных 
                                    объектов контроля, в отношении которых была проведена 
                            ...
                            <xsl:for-each select="cf:controlFactPeriod">
                                ...
                                <p class="under">
                                    ...
                                    проверки, а также дата и время фактического окончания 
                                    документарной проверки, при необходимости указывается 
                                    часовой пояс)
                                </p>
                                <xsl:if test="cf:excludedPeriodsList">
                                    <p class="list">
                                        в срок проведения документарной проверки не включены:
                                    </p>
                                    <xsl:for-each select="cf:excludedPeriodsList/cf:excludedPeriodsListItem">
                                        <p class="list">
                                            с
                                            <span class="data">
                                                <xsl:call-template name="formatdatetime">
                                    ...
                                    <p class="under">
                                        (указываются даты начала и окончания периодов, 
                                        не включаемых в срок документарной проверки)
                                    </p>
                                </xsl:if>
                                <xsl:if test="cf:controlPause">
                                    <xsl:for-each select="cf:controlPause">
                                        <p class="list">
                                            Проведение документарной проверки приостанавливалось
                                        </p>
                                        ...
                                        <p class="under">
                                            ...
                                            документарной проверки, дата и время начала, а также 
                                            дата и время окончания срока приостановления проведения 
                                ...
                                <xsl:for-each select="cf:interactionPeriod">
                                    ...
                                    <p class="under">
                                        (указывается срок (часы, минуты), в пределах которого 
                                        осуществлялось непосредственное взаимодействие с 
                            ...
                            <xsl:for-each select="cf:controlActionsList">
                                <p class="list">
                                    9. При проведении документарной проверки совершены 
                                ...
                                <xsl:for-each select="cf:controlActionsListItem">
                                    <xsl:for-each select="cf:controlActionInfo">
                                        ...
                                        <p class="under">
                                            ...
                                            (надзорное) действие: 1) получение письменных объяснений; 
                                            2) истребование документов; 3) экспертиза)
                                        </p>
                                        <p class="list">
                                            в следующие сроки:
                                        </p>
                                        ...
                                        <p class="under">
                                            (указываются даты и места фактически совершенных 
                                        ...
                                        <xsl:for-each select="cf:controlActionDocs">
                                            ...
                                            <p class="under">
                                                указываются даты составления и реквизиты протоколов и иных 
                                                документов (письменные объяснения, экспертное заключение), 
                            ...
                            <p class="list">
                                10. При проведении документарной проверки 
                                проверочные листы не применялись.
                            </p>
                            <xsl:for-each select="cf:reviewedDocumentsAndAdditionalInfo">
                                <p class="list">
                                    11. При проведении документарной проверки были рассмотрены 
                                    следующие документы и сведения:
                                </p>
                                <xsl:for-each select="cf:documentsList/cf:documentsListItem">
                                    <p class="data">
                                ...
                                <p class="under">
                                    (указываются рассмотренные при проведении документарной 
                                    проверки документы, в том числе: 1) находившиеся в распоряжении 
                                    контрольного (надзорного) органа); 2) представленные контролируемым 
                                    лицом; 3) полученные посредством межведомственного взаимодействия, 
                                    4) иные (указать источник)
                                </p>
                                <xsl:if test="cf:additionalInfoList/cf:additionalInfoListItem">
                                    ...
                                    <p class="under">
                                        (указываются рассмотренные при проведении документарной проверки 
                                        сведения, в том числе: 1) находившиеся в распоряжении контрольного 
                                        (надзорного) органа); 2) представленные контролируемым лицом; 
                                        ) полученные посредством межведомственного взаимодействия, 
                                        4) иные (указать источник)
                                    </p>
                                </xsl:if>
                            </xsl:for-each>
                            <p class="list">
                                12. По результатам документарной проверки установлено:
                            </p>
                                        ...
                                        <xsl:for-each select="cf:identifiedViolationsList/cf:identifiedViolationsListItem">
                                            <p class="data">
                                                ...
                                                <xsl:for-each select="cf:violationDescription">
                                                    <xsl:value-of select="cf:violationDescriptionText"/> 
                                                    <xsl:value-of select="cf:technicalRegulationsList/cf:technicalRegulationsListItem"/>
                            ...
                            <p class="under" style="text-align:justify">
                                (указываются выводы по результатам проведения документарной проверки:
                                <br/><br/>
                                ...
                                являющихся доказательствами нарушения обязательного требования), о несоблюдении 
                                (нереализации) требований, содержащихся в разрешительных документах, с указанием 
                                реквизитов разрешительных документов, о несоблюдении требований документов, 
                                исполнение которых является обязательным в соответствии с законодательством 
                                Российской Федерации, о неисполнении ранее принятого решения контрольного 
                                (надзорного) органа, являющихся предметом документарной проверки;
                                <br/><br/>
                                3) сведения о факте устранения нарушений, указанных в пункте 2, если 
                                нарушения устранены до окончания проведения контрольного надзорного 
                                (мероприятия))
                            </p>
                            <xsl:if test="cf:attachedEntitiesList">
                                ...
                                <p class="under">
                                    (указываются протоколы и иные документы (письменные объяснения, 
                                    экспертное заключение), составленные по результатам проведения 
                                    контрольных (надзорных) действий (даты их составления и реквизиты), 
                                    заполненные проверочные листы (в случае их применения), а также 
                    ...
                    <p class="list">
                        В случае несогласия с настоящим решением Вы можете обжаловать 
                        его в течение 30 календарных дней со дня получения информации 
                        о принятии обжалуемого решения (статья 40 Федерального закона 
                        "О государственном контроле (надзоре) и муниципальном контроле 
                        в Российской Федерации") с использованием единого портала 
                        государственных и муниципальных услуг (функций), перейдя по 
                        ссылке https://knd.gosuslugi.ru/
                    </p>
                </xsl:for-each>
            </body>
        </html>
