...
public class PermissionService {
    ...
    /**
     * ADMIN — да; CUSTOMER — по {@code can_approve}; CONTRACTOR — нет.
     */
    public boolean canApprove(UserEntity user, String documentTypeCode) {
        if (user == null) {
            return false;
        }
        if (ROLE_ADMIN.equals(user.getRole())) {
        ...
        if (ROLE_CUSTOMER.equals(user.getRole())) {
            return permissionRepository
                    .findByRoleAndDocumentType_TypeCode(user.getRole(), documentTypeCode)
                    .map(RoleDocumentPermissionEntity::isCanApprove)
                    .orElse(false);
        }
        return false;
    }
